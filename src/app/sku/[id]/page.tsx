  ...
  const allImages = useMemo(() => {
    ...
    if (product.images) {
      for (const image of product.images) {
        if (image.url && !images.includes(image.url)) images.push(image.url);
      }
    }
    return images;
  }, [product]);
      ...
      {allImages.length > 1 && (
        ...
        >
          {allImages.map((url, index) => (
            <button
              key={index}
              data-testid={`sku-thumbnail-button-${index}`}
              onClick={() => setCurrentIndex(index)}
              aria-label={t("sku.viewImage", { index: index + 1 })}
...
function SkuDetailContent({ id: explicitId }: SkuDetailContentProps) {
  const { t, i18n } = useTranslation("details");
  const id =
    explicitId ??
    (typeof window !== "undefined"
      ? decodeURIComponent(window.location.pathname.match(/^\/sku\/([^/]+)/)?.[1] ?? "")
      : "");
  const isEnglish = isEnglishLanguage(i18n.resolvedLanguage);

  const product = useMemo(() => getProductById(Number(id)), [id]);

  useMemo(() => (product ? getBrandByPinyin(product.brandPinyin) : undefined), [product]);
  ...
  const specs = [
    ...
    {
      label: "Boxes / Carton",
      value: product.boxesPerCarton ? `${product.boxesPerCarton} boxes` : undefined,
    },
            ...
            <div className="space-y-8">
              <div>
                ...
                {product.packPrice && (
                  <div className="mt-6 flex items-baseline gap-3">
                    ...
                    <span className="text-[14px] text-muted-foreground/50 font-medium">
                      ≈ {formatCurrency(Math.round(product.packPrice * usdRate), "USD")}
                    </span>
                  </div>
                )}
                ...
                <div className="mt-6">
                  <div className="flex gap-2">
                    ...
                    >
                      <span className="text-lg leading-none opacity-40" aria-hidden="true">
                        ☆
                      </span>
                      {t("sku.favorite")}
                    </Button>
              ...
              {(product.boxBarcode || product.cartonBarcode) && (
                <div className="space-y-2.5">
                  ...
                  {product.boxBarcode && (
                    <div
                      data-testid="sku-box-barcode-row"
                      className="museum-inline-panel flex items-center gap-3 px-5 py-3 bg-secondary/20"
                    >
                  ...
                  {product.cartonBarcode && (
                    <div
                      data-testid="sku-carton-barcode-row"
                      className="museum-inline-panel flex items-center gap-3 px-5 py-3 bg-secondary/20"
                    >
                      <span className="text-[11px] text-muted-foreground/50 w-28 shrink-0 uppercase tracking-wider">
                        {t("sku.carton")}
