...
export default function MakersPage() {
  ...
  const filteredManufacturers = useMemo(() => {
    let result = manufacturers.filter((manufacturer) => manufacturer.name && !manufacturer.name.includes("未知"));
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(
        (manufacturer) =>
          ...
          manufacturer.brands.some((brand) => brand.toLowerCase().includes(term)),
      );
    }

    if (regionFilter !== "all") {
      result = result.filter(
        (manufacturer) => getManufacturerRegion(manufacturer.name) === regionFilter,
      );
    }
    ...
    return result;
  }, [regionFilter, searchTerm, sortBy]);
